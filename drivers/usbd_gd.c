...
static bscp_usbd_handle_t m_usbd_handle = { .driver.device_specific =
		&m_core_handle, .driver.transmit = &usbd_gd32_transmit,
		.driver.set_address = &usbd_gd32_set_address, .driver.ep_set_stall =
				&usbd_gd32_ep_set_stall, .driver.ep_clear_stall =
				&usbd_gd32_ep_clear_stall, .driver.ep_open = &usbd_gd32_ep_open,
		.driver.ep_close = &usbd_gd32_ep_close, };
...
uint8_t usbd_setup_transaction(usbd_core_handle_struct *pudev) {
	bscp_usbd_handle_request(&m_usbd_handle,
			(usb_setuprequest_t*) pudev->setup_packet);
	...
	if (0U == ep_num) {
		...
		if (USBD_CONFIGURED == pudev->status) {
			...
			if (recv == m_usbd_handle.ep_out[0x7F & ep_num].ep_size) {
				...
				//
				m_usbd_handle.ep_out[ep_num & 0x7F].data_cnt += recv;
				// TODO: Buffer checking!
			} else {
				// Single packet Transfer, or last packet in multi packet transfer
				m_usbd_handle.ep_out[ep_num & 0x7F].data_cnt += recv;
				int received_amount =
						m_usbd_handle.ep_out[ep_num & 0x7F].data_cnt;

				if (received_amount)
				if (m_usbd_handle.ep_out[0x7F & ep_num].data_cb)
					m_usbd_handle.ep_out[0x7F & ep_num].data_cb(&m_usbd_handle,
							ep_num,
							m_usbd_handle.ep_out[ep_num & 0x7F].data_buffer,
							received_amount);

				m_usbd_handle.ep_out[ep_num & 0x7F].data_cnt = 0;

			}
			// Continue or restart transfer
			usbd_ep_rx(pudev, ep_num,
					(uint8_t*) (m_usbd_handle.ep_out[ep_num & 0x7F].data_buffer
							+ m_usbd_handle.ep_out[ep_num & 0x7F].data_cnt),
					m_usbd_handle.ep_out[0x7F & ep_num].ep_size);
		}
	}
	return USBD_OK;
}
	...
	if (0U == ep_num) {
		if (0U != pudev->ctl_count) {
			if (ep->trs_len > ep->maxpacket) {
				...
				if ((0U == (ep->trs_len % ep->maxpacket))
						&& (ep->trs_len < pudev->ctl_count)) {
		...
		m_usbd_handle.ep_in[ep_num & 0x7F].data_cnt += ep->trs_count;

		//if (0U == ep->trs_len) {
		if (m_usbd_handle.ep_in[ep_num & 0x7F].data_size == m_usbd_handle.ep_in[ep_num & 0x7F].data_cnt ) {
			if (USBD_CONFIGURED == pudev->status) {

				if (!(m_usbd_handle.ep_in[ep_num & 0x7F].data_size
						% m_usbd_handle.ep_in[ep_num & 0x7F].ep_size)) {
					// ZLP
					usbd_ep_tx(pudev, ep_num, NULL, 0);
				}
				...
				if (m_usbd_handle.ep_in[0x7F & ep_num].data_cb)
					m_usbd_handle.ep_in[0x7F & ep_num].data_cb(&m_usbd_handle,
							ep_num,
							m_usbd_handle.ep_in[ep_num & 0x7F].data_buffer,
							m_usbd_handle.ep_in[ep_num & 0x7F].data_size);
			}
		} else {
			//usbd_ep_tx(pudev, ep_num, ep->trs_buf, (uint16_t)ep->trs_len);
			usbd_ep_tx(pudev, ep_num,
					m_usbd_handle.ep_in[ep_num & 0x7F].data_buffer
							+ m_usbd_handle.ep_in[ep_num & 0x7F].data_cnt,
					m_usbd_handle.ep_in[ep_num & 0x7F].data_size
							- m_usbd_handle.ep_in[ep_num & 0x7F].data_cnt);
...
int usbd_gd32_transmit(void *hpcd, uint8_t ep_num, void *data, size_t size) {
	usbd_ep_tx(hpcd,  ep_num, data, size);
	return 0;
}
...
int usbd_gd32_ep_open(void *hpcd, uint8_t epnum, uint8_t epsize, uint8_t eptype) {
	...
	if (!(epnum & 0x80))
		usbd_ep_rx(hpcd, epnum, m_usbd_handle.ep_out[epnum & 0x7F].data_buffer,
